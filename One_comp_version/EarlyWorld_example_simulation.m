...
for n=Ns_start:Ns_final
    ...
    omega_a=(2*pi)/Ta; 
    alpha_arr=alpha0+A_a*sin(omega_a*(1:tmax*2)); % --- IMPORTANT: Make the array of alpha and beta twice as large as tmax beacuse when finding the length 20 sequence, the tmax is t_found+tmax,
                                                  % and we are going to have as much as a simulation lasting tmax*2 time steps (artificial limit, of course)
    omega_b=(2*pi)/Tb;
    beta_arr=beta0+A_b*sin(omega_b*(1:tmax*2));
    
    % -- Run the simulation
    [Lt,Lt_OCR,Lt_bl,Lt_E,tf,Ut,Ut_OCR,Ut_E,Ct,Ct_OCR,Ct_bl,Ct_E,Ot_C,Ot_U,Activity,R_CELL,C_CELL,snapshot_found,snapshot_tmax,length_20]=main(A,D,N,L,tmax,beta_arr,alpha_arr,str2num(polymer_size));
    ...
    save(strcat(path,File_Name),'Lt','Lt_OCR','Lt_bl','Lt_E','tf','Ut','Ut_OCR','Ut_E','Ct','Ct_OCR','Ct_bl','Ct_E','Ot_C','Ot_U','Activity','R_CELL','C_CELL','seed','simu_time','snapshot_found','snapshot_tmax','length_20')

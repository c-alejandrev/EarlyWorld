function EarlyWorld_example_simulation_C(Simulation_Name, alpha0, beta0, A_arr, T_arr, polymer_size, Ns_start, Ns_final, interval, tmax, m_t)
...
m_t=str2num(m_t); % Number of molecules that fall to the clay at each time step, representing the concentration of the pool (more concentration means molecules collide with the clay more frequently)
...
for n=Ns_start:Ns_final
    ...
    disp(['Simulation ',num2str(n), ', tmax ',num2str(tmax),', a0 ',num2str(alpha0),', b0 ',num2str(beta0),', Aa ',num2str(A_a),', Ab ',num2str(A_b),', Ta ',num2str(Ta),', Tb ',num2str(Tb),', m_t ',num2str(m_t)]);
    ...
    % -- Run the simulation
    [Lt,Lt_OCR,Lt_bl,Lt_E,tf,Ut,Ut_OCR,Ut_E,Ct,Ct_OCR,Ct_bl,Ct_E,Ot_C,Ot_U,Activity,R_CELL,C_CELL]=main_C(A,D,N,L,tmax,beta_arr,alpha_arr,str2num(polymer_size),m_t);

    % -- Save the simulation
    simu_time=toc;    
    File_Name = strcat('m_t_',num2str(m_t),'__Sim_','beta0_',num2str(beta0),'_alpha0_',num2str(alpha0),'_AaAbTaTb_',num2str(A_a),'_',num2str(A_b),'_',num2str(Ta),'_',num2str(Tb),'_tmax_',num2str(tmax),'_L_',num2str(L),'_Ns_',num2str(n),'.mat'); 

function [Lt,Lt_OCR,Lt_bl,Lt_E,tf,Ut,Ut_OCR,Ut_E,Ct,Ct_OCR,Ct_bl,Ct_E,Ot_C,Ot_U,Activity,R_CELL,C_CELL]=main_C(A,D,N,positions,t,beta,alpha,polymer_size,m_t) 
% -- Here beta and alpha are vectors, each position been a different value for a particular timestep
% -- m_t is a variable that represents the number of molecules that fall to the clay in one time step <<<<<<
...
for i=1:t
    ...
    for m=1:m_t % At least 1 molecule falls to the clay in one time step

function [tf,Ut,Ut_OCR,Ut_E,Ct,Ct_OCR,Ct_bl,Ct_E,Ot_C,Ot_U,Activity,R_CELL,C_CELL]=main_alphabets(A,D,N,positions,t,beta,alpha,polymer_size,alphabet) 
...
L=cell(4,sum(N)); % Create a cell, that is a sort of matrix like structure than can harbor any data type. Here they are all strings. 
                  ...
                  % Row 4 => The exact copy lablels. 'o' if it is the
                  % original, 'b' if they come from the original and 'm' if
                  % they don't come from the original.
...
%---------------------------------------------------------------------------------
%-- NEW!: Create a new variable to track exact nucleotides that come from original
%---------------------------------------------------------------------------------

% Lt_E=''; % E (labels of Exact copies) in pool
Ct_E=''; % E in clay
Ut_E=''; % E in level 1
...
for i=1:size(N,2) % For each type of nucleotide (4 normally)
    for n=1:N(i) % For each of the nucleotides of the same kind (2*template lenght normally)
        ...
        L{2,l}='O';
        L{3,l}='X';
        L{4,l}='!'; %Initialize all nts in pool with 'm' label of exact copies, cause they don't come from the original.
        l=l+1;
    end
end

C=''; C_OCR='';C_bl=''; C_E='';% Remember Lambda is the length of the total number of positions in clay C
C(1:Lambda)='-'; C_OCR(1:Lambda)='-'; C_bl(1:Lambda)='-'; C_E(1:Lambda)='-'; % --- NEW! Variable C_E is also new and it is here for the track of the exact copies at each time step

U=''; U_OCR=''; U_E='';
U(1:Lambda)='-';U_OCR(1:Lambda)='-'; U_E(1:Lambda)='-'; % --- NEW! Variable C_E is also new and it is here for the track of the exact copies at each time step
...
%----------------------------------------------------------------------
%-- NEW!: Create random polymer and place it in random position in clay
%----------------------------------------------------------------------
...
for nt=place_first_polymer:(place_first_polymer+polymer_size-1) %this sould be place_first_polymer+polymer_size-1, cause if not, the size is S0+1.
    cont=cont+1;
    C(nt)=A(randi(size(A,2))); % Create random polymer with four letters' alphabet 'augc'
    C_OCR(nt)='O';
    O_C(nt)=1;
    C_E(nt)=ascii(cont); % --- NEW! '123456...S0' for the original initial polymer.
            ...
            elseif sum(O_C(Pi))==size(R1,2) % If all positions of level 0 are occupied, but none of level 1 is occupied, R1 can fall in level 1 only if it is ssRNA.
                ...
                if sum(O_U(Pi))==0 & is_dsRNA==false % Necessary conditions: available place at level 1 and only ssRNA can attach in level 1
                    ...
                    if RC==1
                         ...
                         Rup = rupture_alphabets(beta(i),R1,R2,alphabet);
                         counter_rand=counter_rand+1;
...
%                 diff_empty_pos=diff(empty_pos);
                ...
                if size(find(diff_empty_pos>0),2)>1 %If we have for example: oligo_r = '---auuagg----cgaguag-'
                        ...
                        for z=1:temps
                            if z==1 % First position where there is not a link => First oligo ends here.
                                if k(z)>1
                                    ...
                                    noligo_r=oligo_r(1:k(1)-1);
                                    comp_oligo_l=oligo_l(1:k(1)-1);
                                    Rup = rupture_alphabets(beta(i),noligo_r,comp_oligo_l,alphabet); 
                                    counter_rand=counter_rand+1;
                                ...
                                if (k(z+1)-k(z))>1 % Evaluate second oligo, because it is not evaluated in following loops
                                    ...
                                    noligo_r=oligo_r((k(z)+1):(k(z+1)-1));
                                    comp_oligo_l=oligo_l((k(z)+1):(k(z+1)-1));
                                    Rup = rupture_alphabets(beta(i),noligo_r,comp_oligo_l,alphabet);  
                                    counter_rand=counter_rand+1;
                            ...
                            elseif z==temps
                                if k(temps)<size(oligo_l,2)
                                    ...
                                    noligo_r=oligo_r(k(temps)+1:size(oligo_l,2));
                                    comp_oligo_l=oligo_l(k(temps)+1:size(oligo_l,2));
                                    Rup = rupture_alphabets(beta(i),noligo_r,comp_oligo_l,alphabet); 
                                    counter_rand=counter_rand+1;
                                    if Rup==1
                    ...
                    if size_rup~=0
                        ...
                        noligo_r=oligo_r(oligo_r~='-');
                        comp_oligo_l=oligo_l(oligo_r~='-');
                        Rup = rupture_alphabets(beta(i),noligo_r,comp_oligo_l,alphabet);  
                        counter_rand=counter_rand+1;
            ...
            for z=1:temps
                if z==1 % First position where there is not a link => First oligo ends here.
                    if k(z)>1
                        ...
                        nt_chain=U(1:k(1)-1);
                        comp_nt_chain=C(1:k(1)-1);
                        Rup = rupture_alphabets(beta(i),nt_chain,comp_nt_chain,alphabet);  
                        counter_rand=counter_rand+1;
                ...
                elseif z==temps
                    if k(temps)<Lambda
                        ...
                        comp_nt_chain=C(k(temps)+1:Lambda);
                        Rup = rupture_alphabets(beta(i),nt_chain,comp_nt_chain,alphabet);
                        counter_rand=counter_rand+1;
                        if Rup==1
...
% ---- Uncomment following lines to obtain all possible data from simulations (althoug notice that this will require additional time per simulation)----
...
%             LEtemp=tempLE2; %Lbltemp is like this: '|m|m|m|bb|m|mbb|...', where | separates the different oligos/nts.
%         end
